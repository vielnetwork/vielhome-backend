...
const CONFIGURED = { apiKey: 'SG.test-key', fromAddress: 'noreply@vielhome.example', fromName: 'VielHome' };
...
describe('EmailProviderService', () => {
  ...
  describe('isConfigured', () => {
    it('is false when apiKey or fromAddress is empty', () => {
      ...
      expect(
        new EmailProviderService(makeConfigService({ apiKey: 'x' })).isConfigured(),
      ).toBe(false);
      expect(
        new EmailProviderService(
          makeConfigService({ ...CONFIGURED, apiKey: '' }),
        ).isConfigured(),
      ).toBe(false);
    });
    ...
    it('is true once apiKey and fromAddress are both set (fromName is optional)', () => {
      expect(
        new EmailProviderService(
          makeConfigService({ apiKey: CONFIGURED.apiKey, fromAddress: CONFIGURED.fromAddress, fromName: '' }),
        ).isConfigured(),
      ).toBe(true);
    });
  });
  ...
  describe('send', () => {
    it('POSTs to SendGrid v3/mail/send with a bearer API key and the exact SendGrid request shape', async () => {
      ...
      await service.send({ to: 'owner@example.com', subject: 'Charge published', body: 'Your charge is ready.' });
    ...
    it('throws ProviderHttpError on a non-2xx response', async () => {
      global.fetch = jest
        .fn()
        .mockResolvedValue({ ok: false, status: 401, statusText: 'Unauthorized', text: async () => '{"errors":[{"message":"bad key"}]}' }) as unknown as typeof fetch;

      const service = new EmailProviderService(makeConfigService(CONFIGURED));
      await expect(
        service.send({ to: 'x@example.com', subject: 's', body: 'b' }),
