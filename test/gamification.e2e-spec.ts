...
async function grantGamificationAnalyticsAdminToStaff(
  prisma: PrismaService,
  personId: string,
): Promise<string> {
  ...
  const role =
    (await prisma.role.findUnique({ where: { name: 'Gamification Analytics Admin (e2e)' } })) ??
    (await prisma.role.create({
      data: { name: 'Gamification Analytics Admin (e2e)', description: 'e2e fixture (ADR-102).' },
    }));
...
describe('Gamification (e2e) — Analytics Staff Access (ADR-047, PlatformRolesGuard)', () => {
  ...
  beforeAll(async () => {
    ...
    adminGrantId = await grantGamificationAnalyticsAdminToStaff(prisma, admin.personId);
