...
async function grantSupportAdminToStaff(
  prisma: PrismaService,
  personId: string,
): Promise<string> {
  ...
  const role =
    (await prisma.role.findUnique({ where: { name: 'Support Admin (e2e)' } })) ??
    (await prisma.role.create({
      data: { name: 'Support Admin (e2e)', description: 'e2e fixture (ADR-102).' },
    }));

  for (const key of ['SUPPORT_VIEW', 'SUPPORT_MANAGE'] as const) {
    const permission =
...
describe('Support & Operations Center (e2e) — Member Lifecycle & Ownership Gate (07.05)', () => {
  ...
  beforeAll(async () => {
    ...
    reviewerGrantId = await grantSupportAdminToStaff(prisma, reviewer.personId);
  });
...
describe('Support & Operations Center (e2e) — Staff Queue, Escalation & Merge (07.05)', () => {
  ...
  beforeAll(async () => {
    ...
    adminGrantId = await grantSupportAdminToStaff(prisma, admin.personId);
  });
